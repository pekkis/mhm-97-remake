...
9. **XState 5 conventions (post-pivot)**
   ...
   - **Spawned children + persistence: use `systemId` + `system.get()`, never store actor refs for cross-actor messaging.** `getPersistedSnapshot()` is JSON-serializable — actor refs (with their methods/closures) **do not survive** the round-trip. `sendParent` is also flagged as not recommended in the XState 5 docs (relies on implicit parent lookup; brittle when restored). The supported pattern:
     1. Register the long-lived parent with a `systemId` at the boundary that creates it: `createActor(gameMachine, { systemId: "game", … })` in `app.ts`.
     2. Children call `sendTo(({ system }) => system.get("game"), …)` at fire time. The lookup happens against the rehydrated system, so it works pre- and post-load identically.
     3. Children get back into the parent's view via the parent's own context (e.g. `parlayBets: ActorRefFrom<typeof betMachine>[]`). That field **is** restored deeply by `getPersistedSnapshot()` — what doesn't survive is _cross-tree messaging refs in context_, not the system-level child registry.
   - **Spawned actors restore deeply.** `getPersistedSnapshot()` walks invoked + spawned children and includes their state nodes and contexts. `createActor(machine, { snapshot })` rehydrates the whole tree. Confirmed: parlay bets parked in `placed`, championship-betting wizard mid-flow, notification subtree — all survived save/load in slot 1.
...
- **PR P2-ish (event system + prank phase declarative): ✅ COMPLETE (2026-04-26)** — Event creation, resolution, and prank phases all migrated. Event registry at `src/game/new-events/` (9 events ported so far), prank registry at `src/game/pranks.ts` (4 pranks). Effect interpreter at `src/game/event-effects.ts` with injected `SpawnEventFn`. `event` and `prank` machine states fully wired with `entry`-driven actions. The legacy `src/sagas/phase/{event,event-creation,prank}.ts` and `src/game/events/` survive as REFERENCE-ONLY pending the remaining 87 event ports.

#### Declarative event/prank patterns (locked, established 2026-04-26)

These are the rules every event/prank port must follow.

- **`DeclarativeEvent<TData, TCreationData = BaseEventCreationFields>`** — second generic types the seed passed to `create`. Pranks pass their full `PrankInstance` shape; system events pass just `{ manager: string }`.
- **Three event archetypes:**
  - **Pre-resolved** (`bazookaStrike`, `pirka`): `resolved: true` literal in payload, `create` does all the work, no `options`, no `resolve`. The walker still fires `process`.
  - **Auto-resolve** (`sellNarcotics`, `protest`, `kasino`): `create` returns `resolved: false`, `resolve(ctx, data)` rolls + snapshots, no `options`. Walker resolves then processes in one entry pass.
  - **Interactive** (`jaralahti`, `jobofferPHL`): `options: () => Record<key, label>`, `resolve(ctx, data, value)` snapshots the choice. Walker leaves `resolved: false` events alone; `RESOLVE_EVENT` event drives them.
- **Walker discipline:** the event-phase entry walker iterates `!processed` events. It skips events that are simultaneously `!resolved && def.options` (interactive — wait for user). Pre-resolved events flow through `resolveAndProcess` which detects `stored.resolved` and skips the `resolve` step. **The bug we hit during the port:** an earlier draft of the machine walker tested `!resolved` (mimicking only step 1 of the saga's three-step `autoResolve → wait → processEvents` flow) and never picked up pre-resolved events. The saga itself was correct — its `processEvents()` stage handled `resolved && !processed`. The new walker collapses all three saga steps into one immer pass; same semantics, no resolved-but-unprocessed gap.
- **Random discipline (mandatory):** every random roll happens in `resolve`. The result is snapshotted onto the payload (e.g. `skillLost`, `caught`, `success`). `process` is purely deterministic over `(ctx, data)`. **Why:** replaying `process` from a saved snapshot must reproduce the same effects; `process` runs after restore.
- **`spawnEvent` EventEffect with injected `SpawnEventFn`:** the effect interpreter (`applyEffects(draft, effects, spawn)`) takes a `SpawnEventFn = (draft, eventId, seed) => void` parameter. The machine layer owns the registry and provides the closure. Avoids circular imports (event files import from `event-effects.ts`; the registry can't live there).
- **Pranks are declarative too:** `DeclarativePrank.execute(ctx, prank) => EventEffect[]`. Most pranks return `[{type: "spawnEvent", eventId, seed: prank}]`; `fixedMatch` returns a direct `addTeamEffect`. The prank-phase action loops `draft.prank.pranks`, calls `applyEffects` for each, then clears the queue. **No saga involved.**
- **Two new event-machine inputs:**
  - `RESOLVE_EVENT { id, value }` — interactive event resolution; component sends `game.send({ type: "RESOLVE_EVENT", payload: { id, value } })`.
  - Auto-resolve happens on `event` state `entry`, no event needed.
- **`SnapshotSelector<T> = (snap: SnapshotFrom<typeof gameMachine>) => T`** — new selector form for predicates that depend on the **state node** in addition to context. `advanceEnabled` is the canonical example: `!snap.matches({ in_game: { executing_phases: "event" } }) || allEventsResolved(snap.context)`. Components consume via `GameMachineContext.useSelector(advanceEnabled)`. Use `ContextSelector<T>` for context-only predicates (most cases); reach for `SnapshotSelector` only when `state.matches(...)` is needed.
- **Calendar `createRandomEvent`/`pranks` flags are redundant.** The phase guard (`has_phase("event_creation")`, `has_phase("prank")`) handles gating. Removed from playoff round entries during this work.
