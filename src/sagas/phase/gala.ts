...
export default function* galaPhase() {
  ...
  const phlFinals = yield* select(
    (state: RootState) => state.game.competitions.phl.phases[3].groups[0]
  );
  ...
  const divFinals = yield* select(
    (state: RootState) => state.game.competitions.division.phases[3].groups[0]
  );

  const divFinalists = divFinals.teams.slice(0, 2).map((t) => teams[t]);

  const divRegularSeason = yield* select(
    (state: RootState) => state.game.competitions.division.phases[0].groups[0]
  );
  ...
  if (
    (phlRegularSeason.stats as TeamStat[]).findIndex(
  ...
  for (const divFinalist of divFinalists) {
    const ranking = (divRegularSeason.stats as TeamStat[]).findIndex(
